...
public class AuctionBidCalculatorImpl implements AuctionBidCalculator {
    ...
    @Override
    public AuctionBidCalculateResult calculateBid(BigDecimal bidPrice, Integer quantity, UUID idempotencyKey, String auctionId, String userId) {
        // --- 입력 스케일 고정 및 저장
        bidPrice = bidPrice.setScale(2, RoundingMode.HALF_UP);
        var metaData =
            new RedisBidData(userId, quantity, bidPrice, idempotencyKey, LocalDateTime.now(clock));

        String stockValue = redis.opsForValue()
            .get(key.auctionOpen(auctionId));
